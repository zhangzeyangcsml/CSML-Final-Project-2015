function [maxstate]=HMMviterbiGSK2(v,phghm,ph1,pdec, psta, pinc)
...
mu(:,T)=ones(H,1);
for t=T:-1:2
	tmp = repmat([psta(v(t),v(t-1)),pinc(v(t),v(t-1)),psta(v(t),v(t-1)),pdec(v(t),v(t-1))]'.*mu(:,t),1,H).*phghm; %max over h(t) values.  
...
for t=2:T % backtrack for t equals to 2 or above
	tmp = [psta(v(t),v(t-1)),pinc(v(t),v(t-1)),psta(v(t),v(t-1)),pdec(v(t),v(t-1))]'.*phghm(:,hs(t-1));
