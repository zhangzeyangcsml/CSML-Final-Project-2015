function [maxstate]=HMMviterbiGSK5(v,phghm,ph1,pdec, psta, pinc,pdec2,psta2,pinc2)
...
%Recursion from T to t=3 step. 
for t=T:-1:3
	tmp = repmat([psta2(v(t),v(t-2)),pinc2(v(t),v(t-2)),psta2(v(t),v(t-2)),psta2(v(t),v(t-2)),psta2(v(t),v(t-2)),pdec2(v(t),v(t-2))]'.*mu(:,t),1,H).*phghm; %max over h(t) values. 
    % We can see from above that it exapnds a column of stable state distribution from Model 4, as we expand one more day of duration model in Model 5  
	mu(:,t-1)= condp(max(tmp)'); % normalise to avoid underflow
end

% Arrangement for the t=2 step to get mu(t=1)
tmp = repmat([psta(v(2),v(1)),pinc(v(2),v(1)),psta(v(2),v(1)),psta(v(2),v(1)),psta(v(2),v(1)), pdec(v(2),v(1))]'.*mu(:,2),1,H).*phghm; %max over h(2) values.  
...
for t=2 % backtrack for t=2 
	tmp = [psta(v(t),v(t-1)),pinc(v(t),v(t-1)),psta(v(t),v(t-1)),psta(v(t),v(t-1)),psta(v(t),v(t-1)),pdec(v(t),v(t-1))]'.*phghm(:,hs(t-1));
	[~, hs(t)]=max(tmp.*mu(:,t));
end

for t=3:T % backtrack for t=3
	tmp = [psta2(v(t),v(t-2)),pinc2(v(t),v(t-2)),psta2(v(t),v(t-2)),psta2(v(t),v(t-2)),psta2(v(t),v(t-2)), pdec2(v(t),v(t-2))]'.*phghm(:,hs(t-1));
	[~, hs(t)]=max(tmp.*mu(:,t));
end
